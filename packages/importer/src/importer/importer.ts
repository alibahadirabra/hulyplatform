...
export class WorkspaceImporter {
  ...
  async updateProject (projectId: Ref<Project>, project: ImportProject): Promise<Ref<Project>> {
    ...
    const projectData = {
      ...
      icon: codePoint === undefined ? undefined : view.ids.IconWithEmoji,
      defaultIssueStatus: project.defaultIssueStatus !== undefined
  ...
  async createIssue (
    ...
    const collabId = await this.createCollaborativeContent(issueId, 'description', content, spaceId)
  ...
  private async getNextIssueIdentifier (project: Project, spaceId: Ref<Project>): Promise<{ number: number, identifier: string }> {
  ...
  private async createAttachment (
    ...
    const uploadResult = await this.fileUploader.uploadFile(id, id, file)
    if (!uploadResult.success) {
      this.logger.error('Failed to upload attachment file: ' + file.name)
      return null
    }
  ...
  private async createCollaborativeContent (
    ...
    await this.fileUploader.uploadCollaborativeDoc(id, collabId, yDoc)
    return collabId
  }
